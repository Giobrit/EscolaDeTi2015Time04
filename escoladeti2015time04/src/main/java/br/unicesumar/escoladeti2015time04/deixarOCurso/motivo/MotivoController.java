...
public class MotivoController {
    ...
    public ResultadoListagem<Motivo> listar(@RequestBody RequisicaoListagem requisicaoListagem) {
        return this.service.listar(requisicaoListagem);
    }

    @RequestMapping(method = RequestMethod.POST)
    public void criarMotivo(@RequestBody Motivo motivo) {
        this.service.criar(motivo);
    }

    @RequestMapping(method = RequestMethod.PUT)
    public void editarMotivo(@RequestBody MotivoCommandEditar motivo) {
        this.service.editar(motivo);
    }

    @RequestMapping(value = "{id}/{status}", method = RequestMethod.PUT)
    public void alterarStatus(@PathVariable Long id, @PathVariable br.unicesumar.escoladeti2015time04.usuario.Status status) {
