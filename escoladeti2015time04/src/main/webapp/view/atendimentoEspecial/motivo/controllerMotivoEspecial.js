AppModule.controller("controllerFormMotivoAtendimentoEspecial", controllerFormMotivoAtendimentoEspecial);

AppModule.controller("controllerFormMotivoAtendimentoEspecial", controllerFormMotivoAtendimentoEspecial);

function controllerFormMotivoAtendimentoEspecial($scope, $http, $routeParams, $location, growl, $timeout) {
    ...
    $scope.salvar = function () {
        if ($scope.editando) {
            $http.put("/atendimento/especial/motivo", $scope.motivo).success(onSuccess).error(onError);
        } else {
            $http.post("/atendimento/especial/motivo", $scope.motivo).success(onSuccess).error(onError);
        }

        function onSuccess() {
            $timeout(success, 100);
            $location.path("/AtendimentoEspecial/Motivo/list");
        }
        function success() {
    ...
    $scope.editar = function (id) {
        $http.get("/atendimento/especial/motivo/" + id).success(onSuccess).error(onError);
...
function controllerListMotivoAtendimentoDeixarOCurso($scope, $http, growl) {
    ...
    $scope.listar = function () {
        ...
        $http.post("/atendimento/especial/motivo/listar", requisicaoListagem).success(onSuccess).error(onError);
        function onSuccess(data) {
